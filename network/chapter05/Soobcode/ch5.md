...
#### 정적 vs 동적
정적 : 경로가 아주 느리게 변함. 사람이 직접 수정. (경로를 사람이 고정해두냐, 네트워크가 상황에 따라 자동으로 바꾸냐 차이)

동적 : 트래픽/토폴로지 변화에 자동 대응. 빠른 대응 가능하지만 루프나 진동 문제에 취약. (라우터들이 서로 정보를 주고받으면서 경로를 자동으로 계산하고 갱신하는 방식.)
...
부하 민감 : 링크 혼잡 수준에 따라 비용이 동적으로 변함. 혼잡한 링크를 우회하는 경향. (현재 네트워크가 얼마나 혼잡한지를 보고 링크 비용을 바꾸는 방식)

부하 비민감 : 오늘날 인터넷 라우팅(RIP, OSPF, BGP)이 사용하는 방식. 링크 비용이 현재 혼잡을 반영하지 않음. (현재 혼잡 상태를 링크 비용에 반영하지 않는 방식). 링크 비용은 관리자가 설정한 값이나 고정된 기준에 따라 유지되며, 현재 트래픽이 많다고 해서 즉시 비용이 변하지 않는다.

```
왜 오늘날 인터넷은 부하 비민감 방식을 많이 쓸까? -> (진동 문제)

트래픽이 몰리면 비용이 올라가고, 그러면 라우터들이 다른 경로로 우르르 이동할 수 있어.
...
z가 y를 통해 x로 가고 있다면, z는 y에게 "x까지의 거리가 무한대"라고 거짓 정보를 전달. 
y가 z를 통해 x로 가려는 시도를 차단.
...
| 구분 | LS(Link State) 알고리즘 | DV(Distance Vector) 알고리즘 |
|---|---|---|
| 기본 개념 | 링크 상태 정보를 전체 네트워크에 공유하고, 각 라우터가 전체 토폴로지를 바탕으로 최단 경로를 계산 | 이웃 라우터와 거리 정보를 교환하면서 목적지까지의 최소 비용을 점진적으로 계산 |
| 정보 범위 | 네트워크 전체 토폴로지와 링크 비용을 알아야 함 | 처음에는 직접 연결된 이웃 정보만 알고 시작 |
| 정보 교환 대상 | 모든 라우터에게 링크 상태 정보 전파 | 직접 연결된 이웃 라우터와만 정보 교환 |
| 사용 알고리즘 | 다익스트라 알고리즘 | 벨만-포드 알고리즘 |
| 계산 방식 | 각 라우터가 전체 지도를 보고 자신 기준의 최단 경로 계산 | 이웃을 거쳐 목적지로 가는 비용 중 최솟값 선택 |
| 동작 특징 | 중앙 집중형 성격: 전체 정보를 기반으로 계산 | 분산형 성격: 이웃과 반복적으로 정보를 교환하며 계산 |
| 메시지 복잡도 | 링크 상태 정보를 전체에 전파해야 하므로 메시지 부담이 큼 | 이웃끼리만 정보를 교환하므로 한 번의 교환 범위는 작음 |
| 수렴 속도 | 전체 정보를 알고 계산하므로 비교적 빠르고 명확하게 수렴 | 상황에 따라 수렴이 느릴 수 있음 |
| 문제점 | 링크 비용이 부하에 따라 변하면 진동 문제가 발생할 수 있음 | 라우팅 루프, 무한 계수 문제(count-to-infinity)가 발생할 수 있음 |
| 오류 영향 | 한 라우터의 계산 오류는 주로 해당 라우터의 포워딩 테이블에 영향 | 잘못된 정보가 이웃을 통해 다른 라우터로 퍼질 수 있음 |
| 대표 프로토콜 | OSPF | RIP |
| 비유 | 모든 라우터가 전체 지도를 보고 각자 최단 경로를 계산 | 이웃에게 “목적지까지 얼마나 걸려?”라고 물어보며 경로를 찾음 |
